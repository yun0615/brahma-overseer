...
function handleResponseRunning(resp, socket, pbuf) {
    switch(resp.type) {
        ...
        case pbuf.Response.ResponseType.SCREENINFO:
            ...
            window.screeninfoX = resp.screenInfo.x/2;
            window.screeninfoY = resp.screenInfo.y/2;
            setTimeout(function() {
...
function handleTouch(ev, socket, pbuf, touchtype) {
	...
	//    scaledY = window.currtouches[i].pageY - rect.top;    
	    scaledX = (window.currtouches[i].pageX - rect.left)*2;
            scaledY = (window.currtouches[i].pageY - rect.top)*2;
        }
