...
aws.init = function () {
    var awsInfo = brahma.config.get('aws');
    if (!awsInfo) {
        brahma.logger.error("Failed to initialize AWS: cloud_platform set to 'aws', but no AWS config (aws) found!");
        process.exit(1);
    }
...
aws.createAndStartVM = function (user, image, flavor) {
    var deferred = Q.defer();
    var vmObj = {
        //name: "brahma-user-instance_" + user.username,
        flavor: flavor,
        image: image,
        zone: brahma.config.get('aws:availabilityZone')
    };
        ...
        if (err) {
            ...
            server.setWait({status: server.STATUS.running}, poll, function (err) {
                if (err) {
                    ...
                    if (user.vm_ip && user.vm_ip.length > 0) {
                        createNameTag(user.vm_id, "brahma-user-instance_" + user.username + "-" + new Date().getTime() )
                            .then(function (obj) {
...
aws.createVolumeForUser = function (user) {
    ...
    var params1 = {
        AvailabilityZone: brahma.config.get('aws:availabilityZone'),
        SnapshotId: brahma.config.get('new_vm_defaults:goldsnapshotId')
    };
    ...
    aws.ec2.createVolume(params1, function (err, data) {
        if (err) {
            ...
            createNameTag(data.VolumeId, "brahma-user-volume_" + user.username)
                .then(function (obj) {
