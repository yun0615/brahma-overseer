...
openstack.init = function () {
    var openstackInfo = brahma.config.get('openstack');
    if (!openstackInfo) {
        brahma.logger.error("Failed to initialize OpenStack: cloud_platform set to 'openstack', but no OpenStack config (openstack) found!");
        process.exit(1);
    }
...
openstack.setUpUser = function (userObj) {
    ...
    var flvr = brahma.config.get('new_vm_defaults:vmflavor');
    var user = userObj;
    var img = brahma.config.get('new_vm_defaults:images')[user.device_type];
    ...
    if (user.vm_ip && user.vm_ip.length > 0) {
        ...
        openstack.createAndStartVM(user, img, flvr)
            ...
            .then(function (updatedUser){
                ...
                brahma.User.update({username: updatedUser.username}, update, function (err) {
                    if (err) {
...
openstack.createAndStartVM = function (user, image, flavor) {
    var deferred = Q.defer();
    var vmObj = {
        name: "brahma-user-instance_" + user.username + "-" + new Date().getTime(),
        flavor: flavor,
        image: image
    };
    openstack.computeClient.createServer(vmObj, function (err, server) {
        var poll = brahma.config.get('new_vm_defaults:pollintervalforstartup');
        ...
        if (err) {
            ...
            server.setWait({status: server.STATUS.running}, poll, function (err) {
                if (err) {
                    ...
                    if (brahma.config.isEnabled('new_vm_defaults:use_floating_ips')) {
...
openstack.createVolumeForUser = function (user) {
    var deferred = Q.defer();
    var nme = "brahma-user-volume_" + user.username;
    var desc = "Block Storage for: " + user.username;
    var goldSize = brahma.config.get('new_vm_defaults:goldsnapshotSize');
    var goldId = brahma.config.get('new_vm_defaults:goldsnapshotId');
...
openstack.assignFloatingIpToVM = function (user) {
    ...
    if (brahma.config.isEnabled('new_vm_defaults:use_floating_ips')) {
        ...
        var pool = brahma.config.get('new_vm_defaults:floating_ip_pool'); // optional
        ...
        openstack.computeClient.allocateNewFloatingIp(pool, function (err, ip) {
            if (err) {
                ...
                brahma.logger.verbose("Allocated floating IP '%s', ID '%s'", ip.ip, ip.id);
...
function deallocateFloatingIp(user) {
    ...
    openstack.computeClient.deallocateFloatingIp(user.vm_ip_id, function (err) {
        if (err) {
            ...
            brahma.logger.verbose("De-allocated floating IP '%s'", user.vm_ip);
            deferred.resolve(user);
        }
    });
    return deferred.promise;
    ...
    if (obj.vm_id && obj.vm_id.length > 0) {
        openstack.computeClient.destroyServer(obj.vm_id, function (err, id) {
            if (err) {
                ...
                brahma.logger.verbose("Destroyed expired VM, ID '%s'", obj.vm_id);
...
openstack.restartVM = function (obj) {
    ...
    openstack.computeClient.rebootServer(obj.vm_id, {type: 'HARD'}, function (err) {
        if (err) {
            ...
            brahma.logger.verbose("Restarted VM, ID '%s'", obj.vm_id);
            deferred.resolve(obj);
        }
    });
