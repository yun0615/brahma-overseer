    ...
    setInterval(
        function () {
            ...
            getExpiredVmSessions()
                .then(function (obj) {
                    brahma.logger.debug("getExpiredVmSessions returned %d result(s)", obj.length);
                    ...
                    for (var i = 0; i < obj.length; i++) {
                        ...
                        sess.removeQ()
                            .then(function (result) {
                                brahma.logger.verbose("Removed VM session for '%s'", result.username)
                            }).catch(function (err) {
                                brahma.logger.error("Couldn't remove VM session:", err.message);
                            }).done();

                        // obtain and remove the user's VM information, then destroy the VM
                        brahma.User.findOneQ(query)
                            .then(removeUserVM)
                            .then(brahma.cloud.destroyVM)
                            .catch(function (err) {
                                brahma.logger.error("Failed to destroy expired VM:", err.message);
                            }).done();
                    }
                }).catch(function (err) {
                    brahma.logger.error("Failed to process expired VMs:", err.message);
                }).done();
        },
            brahma.config.get('vm_check_interval') * 1000
    );
};
...
function getExpiredVmSessions(req, res) {
    var query = {
        'lastAction': {
            '$lt': toDate(brahma.config.get('vm_idle_ttl')).seconds.ago,
            '$gt': new Date(0) // the lastAction has to be > 0, otherwise the session is active
        }};

    return brahma.VMSession.findQ(query);
