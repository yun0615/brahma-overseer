...
/**
 * Checks for JWT authentication in header with field name: brahma-authtoken
...
exports.checkToken = function (req, res, next) {
    ...
    var token = req.get('brahma-authtoken');
...
exports.makeToken = function (userInfo) {
    return jwt.sign(userInfo, getPrivKey(), {algorithm: brahma.config.get("jwt_signing_alg")});
};
...
exports.loadStrategy = function () {
    if (brahma.config.useTlsCertAuth()) {
        ...
        return function (req, callback) {
            var cert = undefined;
            if (brahma.config.get('behind_reverse_proxy')) {
            ...
            if (cert && cert.subject && cert.subject.CN) {
                brahma.User.findOne({username: cert.subject.CN}, function (err, user) {
                    if (err) {
                        // server error
                        callback(500);
                    } else if (!user) {
        ...
        return function (req, callback) {
            ...
            if (!un || (!pw && !token)) {
                ...
                brahma.User.findOne({username: un ,approved: true}, function (err, user) {
                    if (err) {
                        // server error
                        callback(500);
                    } else if (!user) {
                        // no user found
                        callback(401);
                    } else if (pw && brahma.config.get('authentication_type') === 'pam') {
                        // the user presented a password and PAM authentication is enabled
                        var pam = require('authenticate-pam'),
                            pam_service = brahma.config.get('pam_service'),
                            pam_host = 'localhost';
...
function getPubKey() {
    if (pubKey === null) {
        pubKey = fs.readFileSync(brahma.config.get('server_certificate'));
    }
    return pubKey;
}
