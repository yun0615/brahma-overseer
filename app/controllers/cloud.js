    ...
    if(un) {
        brahma.User.findUserWithPromise({username: un})
            .then(function (userObj) {
                if (!userObj) {
                    brahma.logger.error("setUpVm failed to find user");
                    res.json(404, {msg: "User not found"});
                }
                // before we proceed, validate some user properties first
                else if (!userObj.vm_ip && brahma.config.get('new_vm_defaults:images')[userObj.device_type] === undefined)
                    ...
                    return brahma.cloud.setUpUser(userObj)
                    .then(function (userObj) {
                ...
                brahma.logger.error("setUpVm failed:", err.message);
                res.json(500, {});
            }).done();
    }
};
...
exports.listImages = function (req, res) {
    ...
    brahma.cloud.getFlavors(function (err, allflavors) {

        if (err) {
            brahma.logger.error("listImages failed to get flavors:", err.message);
            ...
            brahma.cloud.getImages(function (err, r) {
                ...
                if (err) {
                    brahma.logger.error("listImages failed to get images:", err.message);
...
exports.listDevices = function (req,res) {
    var obj = brahma.config.get("new_vm_defaults:images");
    res.json(200,obj);
};
...
exports.listVolumes = function (req, res) {
    ...
    brahma.cloud.getVolumes(function (err, r) {
        if (err) {
            brahma.logger.error("listVolumes failed:", err.message);
...
exports.createVolume = function (req, res) {
    ...
    brahma.User.findUserWithPromise(jsonObj)
        .then(function (userObj) {
            if (!userObj) {
                brahma.logger.error("createVolume failed to find user");
                res.json(404, {msg: "User not found"});
            } else {
                return brahma.cloud.createVolumeForUser(userObj)
                .then(function(userObj) {
                    var u = userObj.user;
                    brahma.User.update({username: u.username}, {volume_id: u.volume_id}, function (err, numberAffected, raw) {
                        if (err) {
                            brahma.logger.error("createVolume failed to find user:", err.message);
                            res.json(404, {msg: "User not found"});
                        } else {
                            if (numberAffected === 1) {
                                res.json(200, {});
        ...
        .catch(function (err){
            brahma.logger.error("createVolume failed:", err.message);
...
exports.assignVolume = function (req, res) {
    ...
    if( requestObj.username && requestObj.volid) {
        brahma.User.findUserWithPromise({username: requestObj.username})
            .then(function (userObj) {
                if (!userObj) {
                    brahma.logger.error("assignVolume failed to find user");
                    res.json(404, {msg: "User not found"});
                } else {
                    userObj.volume_id = requestObj.volid;
                    return brahma.User.updateUserWithPromise(userObj)
                    .then(function (u) {
                        res.json(200, {});
                    });
                }
            }).catch(function (err) {
                brahma.logger.error("assignVolume failed:", err.message);
